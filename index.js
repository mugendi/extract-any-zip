...
const fileToBuffer = require('file-to-buffer');

const fromBuffer = promisify(yauzl.fromBuffer);
const pipeline = promisify(stream.pipeline);

async function openZip(dataPathOrURL, opts) {
    try {
        const buf = await fileToBuffer(dataPathOrURL);
        return fromBuffer(buf, opts);
    } catch (error) {
        throw error;
    }
}
...
class Extractor {
    ...
    async extractEntry(entry) {
        ...
        if (symlink) {
            ...
            await pipeline(
                readStream,
                createWriteStream(dest, { mode: procMode })
            );
        }
    }
